...
function VerificationStep1({ onOtpSent }: VerificationStep1Props) {
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
      const response = await apiClient.sendSupervisorOtp(email);
  ...
  return (
    <Card className="max-w-md mx-auto">
      <CardHeader>
        <CardTitle>Supervisor Account Verification</CardTitle>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSendOtp} className="space-y-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Enter the email address provided by your intern's university
            </p>
          </div>
...
function VerificationStep2({ email, onVerificationComplete }: VerificationStep2Props) {
  ...
  const handleVerification = async (e: React.FormEvent) => {
    ...
    try {
      const response = await apiClient.verifySupervisorOtp({
        email,
        otp_code: otpCode,
        password,
...
export function SupervisorVerificationFlow() {
    ...
    return (
      <Card className="max-w-md mx-auto">
        ...
        <CardContent className="text-center">
          <p className="mb-4">Your supervisor account has been activated successfully.</p>
          <Button 
