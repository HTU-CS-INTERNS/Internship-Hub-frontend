...
import type { DailyReport, DailyTask, UserProfileData } from "@/types"; // Ensure these types are correctly defined based on your Prisma schema
  ...
  <Card className="shadow-lg hover:shadow-xl transition-shadow duration-300 rounded-xl dashboard-card flex flex-col">
    <CardHeader className="pb-2">
      <div className="flex items-center justify-between">
        ...
        <div
          className={`w-12 h-12 rounded-full ${iconBgColor} flex items-center justify-center ${iconColor} shrink-0`}
        >
...
const StudentTaskItem: React.FC<{
  ...
  const statusStyles = {
    Completed: {
      ...
      badge:
        "bg-green-100 text-green-700 border-green-500/30 dark:bg-green-900/50 dark:text-green-300 dark:border-green-700/50",
    },
    Pending: {
      icon: CircleDot,
      color: "text-yellow-500 dark:text-yellow-400",
      badge:
        "bg-yellow-100 text-yellow-700 border-yellow-500/30 dark:bg-yellow-900/50 dark:text-yellow-300 dark:border-yellow-700/50",
    },
    Overdue: {
      icon: AlertOctagonIcon,
      color: "text-red-500 dark:text-red-400",
      badge:
        "bg-red-100 text-red-700 border-red-500/30 dark:bg-red-900/50 dark:text-red-300 dark:border-red-700/50",
    },
  };
    ...
    <div className="p-4 flex items-start hover:bg-muted/30 transition-colors">
      ...
      <div className="ml-3 flex-1">
        <div className="flex items-center justify-between">
          ...
          <Badge
            variant="outline"
            className={`text-xs ${statusStyles[status].badge}`}
          >
            {status}
          </Badge>
        </div>
      ...
      {onEdit && (
        <Button
          variant="ghost"
          size="icon"
          className="ml-2 h-7 w-7 self-center text-muted-foreground hover:text-primary"
          onClick={() => onEdit(task)}
        >
...
const StudentDashboard: React.FC = () => {
  ...
  >(null);
  const [myInternshipSubmission, setMyInternshipSubmission] = React.useState<
    PendingInternship | null
  >(null);
  const [activeInternship, setActiveInternship] = React.useState<
    Internship | null
  >(null);
  ...
  const [reportAbuseDialogOpen, setReportAbuseDialogOpen] = React.useState(
    false,
  );
    ...
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        if (user.role === "STUDENT") {
          ...
          const submission = await StudentApiService
            .getMyInternshipSubmission() as PendingInternship | null;
          ...
          if (submission && submission.status === "APPROVED") {
            const internship = await StudentApiService.getMyInternship() as
              | Internship
              | null;
            ...
            if (internship) {
              // Fetch today's tasks
              const tasks = await StudentApiService.getTasks(
                parseInt(internship.id),
                getTodayDateString(),
              ) as DailyTask[];
              setDailyTasks(tasks || []);
  ...
  const handleQuickReportSubmit = async () => {
    if (!reportSummary.trim() || !reportDate) {
      toast({
        title: "Error",
        description: "Date and work summary are required.",
        variant: "destructive",
      });
      return;
    }
    if (!activeInternship) {
      toast({
        title: "Error",
        description: "You need an active internship to submit reports.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: `Summary for ${format(reportDate, "PPP")} recorded.`,
      });
      setReportSummary("");
  ...
  const handleReportAbuseSubmit = async () => {
    ...
    if (!user) {
      toast({
        title: "Error",
        description: "User not identified for reporting.",
        variant: "destructive",
      });
      return;
    }
  ...
  // Calculated stats
  const internshipDurationDays = activeInternship
    ? differenceInDays(
      parseISO(activeInternship.end_date),
      parseISO(activeInternship.start_date),
    ) + 1
    : 0;
  const daysCompleted = activeInternship
    ? differenceInDays(new Date(), parseISO(activeInternship.start_date)) + 1
    : 0;
  const progressPercentage = internshipDurationDays > 0
    ? Math.min(100, Math.round((daysCompleted / internshipDurationDays) * 100))
    : 0;
  ...
  const pendingTasksCount =
    dailyTasks.filter((task) => task.status === "PENDING").length;
    ...
    <>
            ...
            {activeInternship
              ...
              : (
                <Link href="/profile" passHref>
                  <Button
                    variant="destructive"
                    className="font-medium transition rounded-lg px-4 py-2 text-sm animate-pulse"
                  >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <DashboardStatsCard
          ...
          progressLabel={activeInternship
            ? `Total Days: ${internshipDurationDays}`
            ...
            <div className="flex items-center text-xs">
              <div className="flex text-yellow-400">
                ...
                <StarIcon
                  ...
                  stroke="currentColor"
                  strokeWidth={1}
                  className="w-3 h-3 text-yellow-400/80"
                />
              </div>
                ...
                ? (
                  dailyTasks.map((task) => (
                    <StudentTaskItem
                      ...
                      task={task}
                      onEdit={() => {
                        /* Handle edit task, maybe open a dialog */ toast({
                          title: "Edit task",
                          description: "Functionality to be implemented.",
                        });
                      }}
                    />
                  ))
              ...
              {!activeInternship
                ? (
                  <div className="text-center p-4 text-muted-foreground bg-blue-50 rounded-md border border-blue-200">
                    ...
                    <p>
                      You need an active, approved internship to submit reports.
                    </p>
                ...
                : (
                  <>
                    <div className="flex flex-col sm:flex-row gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        <Input
                          ...
                          className="text-xs border-input text-foreground file:text-foreground rounded-lg"
                        />
                      </div>
                    </div>
                    <div>
                      ...
                      <Textarea
                        ...
                        rows={3}
                        className="border-input text-foreground rounded-lg"
                      />
                    </div>
                    <div className="flex justify-center space-x-2">
                      <Button
                        variant="outline"
                        className="border-input text-foreground hover:bg-muted hover:text-muted-foreground rounded-lg"
                        onClick={() =>
                          toast({
                            title: "Draft Saved!",
                            description:
                              "Your quick report draft has been saved.",
                          })}
                      >
                        Save Draft
                      </Button>
                    ...
                    <p className="text-xs text-muted-foreground text-center">
                      For a detailed submission, go to{" "}
                      <Link
          ...
          <Card className="shadow-lg rounded-xl overflow-hidden bg-card text-card-foreground">
            ...
            <CardContent className="p-4">
              <div className="flex flex-col items-center">
                <ChartContainer
                  config={studentCheckInChartConfig}
                  ...
                  <PieChart accessibilityLayer>
                    ...
                    <text
                      ...
                      className={`fill-current ${
                        todayCheckIn ? "text-primary" : "text-muted-foreground"
                      } text-lg font-bold`}
                    >
                ...
                <div className="text-center mt-2">
                  ...
                  {todayCheckIn && (
                    <p className="text-muted-foreground text-xs mt-0.5">
                      {format(new Date(todayCheckIn.check_in_timestamp), "p")}
                      {" "}
                      at{" "}
                      {todayCheckIn.address_resolved ||
                        todayCheckIn.manual_reason || "Unknown location"}
                    </p>
                  )}
                  {!todayCheckIn && (
                    <p className="text-muted-foreground text-xs mt-0.5">
          ...
          <Card className="shadow-lg rounded-xl bg-destructive/10 border-destructive/30 text-destructive-foreground">
            <CardHeader>
              <CardTitle className="font-headline text-lg flex items-center text-destructive">
                <AlertCircle className="mr-2 h-5 w-5" /> Need Help?
              </CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-sm text-foreground">
                If you are facing any issues or grievances at your workplace,
                please report it immediately.
              </p>
            </CardContent>
            <CardFooter>
              <Dialog
                open={reportAbuseDialogOpen}
                onOpenChange={setReportAbuseDialogOpen}
              >
                ...
                <DialogContent className="sm:max-w-[425px]">
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      Your report will be sent to the university administrator
                      for review. Please be as detailed as possible. Your
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="shadow-lg rounded-xl bg-card text-card-foreground">
          <CardHeader className="border-b border-border">
            <CardTitle className="font-headline text-lg flex items-center">
              <Briefcase className="mr-2 h-5 w-5 text-primary" />Industrial
              Supervisor
            </CardTitle>
          </CardHeader>
          <CardContent className="p-4">
              ...
              ? (
                <div className="flex items-center space-x-3">
                  <Avatar className="h-12 w-12 border">
                    <AvatarImage
                      src={activeInternship.company_supervisor.users
                        ?.profile_picture_url || undefined}
                      alt={activeInternship.company_supervisor.users
                        ?.first_name || "Supervisor"}
                    />
                    <AvatarFallback className="bg-primary/20 text-primary">
                      {getInitials(
                        `${activeInternship.company_supervisor.users?.first_name} ${activeInternship.company_supervisor.users?.last_name}`,
                      )}
                    </AvatarFallback>
                  </Avatar>
                  <div>
                    <p className="font-medium text-foreground">
                      {activeInternship.company_supervisor.users?.first_name}
                      {" "}
                      {activeInternship.company_supervisor.users?.last_name}
                    </p>
          ...
          <CardFooter className="p-4 border-t border-border">
            {activeInternship?.company_supervisor
              ? (
                <Link
                  href={`/communication?to=${activeInternship.company_supervisor.users?.id}`}
                  className="w-full"
                >
                  <Button
                    variant="outline"
                    className="w-full rounded-lg border-input text-foreground hover:bg-muted"
                  >
        ...
        <Card className="shadow-lg rounded-xl bg-card text-card-foreground">
          <CardHeader className="border-b border-border">
            <CardTitle className="font-headline text-lg flex items-center">
              <SchoolIcon className="mr-2 h-5 w-5 text-primary" />Assigned
              Lecturer
            </CardTitle>
          </CardHeader>
          <CardContent className="p-4">
            {activeInternship?.lecturer
              ? (
                <div className="flex items-center space-x-3">
                  <Avatar className="h-12 w-12 border">
                    <AvatarImage
                      src={activeInternship.lecturer.users
                        ?.profile_picture_url || undefined}
                      alt={activeInternship.lecturer.users?.first_name ||
                        "Lecturer"}
                    />
                    <AvatarFallback className="bg-primary/20 text-primary">
                      {getInitials(
                        `${activeInternship.lecturer.users?.first_name} ${activeInternship.lecturer.users?.last_name}`,
                      )}
                    </AvatarFallback>
                  </Avatar>
                  <div>
                    <p className="font-medium text-foreground">
                      {activeInternship.lecturer.users?.first_name}{" "}
                      {activeInternship.lecturer.users?.last_name}
                    </p>
                    <p className="text-xs text-muted-foreground">
                      Faculty Lecturer
                    </p>
              ...
              : (
                <div className="text-muted-foreground text-sm py-2">
                  No faculty lecturer assigned yet.
                </div>
              )}
          </CardContent>
          <CardFooter className="p-4 border-t border-border">
            {activeInternship?.lecturer
              ? (
                <Link
                  href={`/communication?to=${activeInternship.lecturer.users?.id}`}
                  className="w-full"
                >
                  <Button
                    variant="outline"
