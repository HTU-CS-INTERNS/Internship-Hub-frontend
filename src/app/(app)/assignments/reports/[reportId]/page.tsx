...
export default function LecturerReportReviewPage() {
  ...
  React.useEffect(() => {
    async function loadData() {
      ...
      if (foundReport && foundStudent) {
        ...
        const initialLecturerComment = (foundReport as any).lecturerComments || ''; // Assuming lecturerComments field
  ...
  const handleSubmitFeedback = async (newStatus: 'APPROVED' | 'REJECTED') => {
    ...
    const updatedReport = await updateReportStatus(report.id, newStatus, feedbackComment, 'lecturer');
  ...
  const handleSaveComments = async () => {
    ...
    const updatedReport = await updateReportStatus(report.id, report.status, feedbackComment, 'lecturer');
        ...
        <div className="lg:col-span-1">
            <Card className="shadow-lg rounded-xl sticky top-24">
                ...
                <CardContent className="space-y-4">
                    {(report.status === 'APPROVED' || report.status === 'REJECTED') && (report as any).lecturerComments && !canProvideFeedback && (<div className="p-3 bg-muted/50 rounded-md border border-input"><p className="text-sm font-medium text-foreground mb-1">Your Feedback:</p><p className="text-sm text-muted-foreground italic">"{(report as any).lecturerComments}"</p></div>)}
                    ...
                    {!canProvideFeedback && !(report as any).lecturerComments && (<p className="text-sm text-muted-foreground">No lecturer feedback provided yet, or report is not in a state that accepts new feedback.</p>)}
                </CardContent>
