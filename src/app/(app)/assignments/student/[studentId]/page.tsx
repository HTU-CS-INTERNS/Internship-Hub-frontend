  ...
  return (
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 space-y-6">
            <Card className="shadow-lg rounded-xl"><CardHeader className="p-6 border-b text-center bg-primary/10"><Avatar className="h-24 w-24 mx-auto mb-3 border-4 border-primary/30 shadow-lg"><AvatarImage src={student.avatarUrl} alt={student.name} data-ai-hint="person student"/><AvatarFallback className="text-3xl bg-muted text-muted-foreground">{getInitials(student.name)}</AvatarFallback></Avatar><CardTitle className="font-headline text-xl text-primary">{student.name}</CardTitle><CardDescription className="text-muted-foreground">{student.email}</CardDescription></CardHeader><CardContent className="p-6 text-sm space-y-3"><p><strong className="text-foreground">Department:</strong> {student.department}</p><p><strong className="text-foreground">Faculty:</strong> {student.faculty}</p><p><strong className="text-foreground">Internship Company:</strong> {companyName}</p>{assignment?.companySupervisor && (<><p><strong className="text-foreground">Industrial Supervisor:</strong> {assignment.companySupervisor}</p><Button variant="outline" size="sm" asChild className="w-full rounded-lg mt-2"><Link href={`/communication?contact=${encodeURIComponent(assignment.companySupervisor)}&company=${encodeURIComponent(assignment.companyName || '')}`}><MessageSquare className="mr-2 h-4 w-4" /> Contact Supervisor</Link></Button></>)}</CardContent></Card>
