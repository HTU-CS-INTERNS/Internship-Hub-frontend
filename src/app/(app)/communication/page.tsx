...
const DUMMY_CONTACTS: Array<Partial<AppUser>> = [
  { id: 'lecturer1', name: 'Dr. Emily Carter', role: 'LECTURER', avatarUrl: 'https://placehold.co/100x100.png' },
  { id: 'supervisor1', name: 'John Smith (Company)', role: 'SUPERVISOR', avatarUrl: 'https://placehold.co/100x100.png' },
  { id: 'hod1', name: 'Prof. Alan Turing', role: 'HOD', avatarUrl: 'https://placehold.co/100x100.png' },
];
...
export default function CommunicationPage() {
  ...
  return (
    <div className="space-y-8 p-4 md:p-6 h-[calc(100vh-theme(spacing.24))] flex flex-col">
      <PageHeader
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 flex-1 min-h-0">
        <Card className="md:col-span-1 shadow-lg rounded-xl flex flex-col overflow-hidden">
          <CardHeader className="border-b">
            <CardTitle className="font-headline text-lg">Contacts</CardTitle>
            ...
            <ScrollArea className="h-full">
              ...
              {DUMMY_CONTACTS.map(contact => (
                <Button
                  ...
                  className="w-full justify-start h-auto py-3 px-4 rounded-md"
        ...
        <Card className="md:col-span-2 shadow-lg rounded-xl flex flex-col overflow-hidden">
          {selectedContact ? (
            <>
              <CardHeader className="border-b">
              ...
              <CardContent className="flex-1 p-0">
                 {/* Adjusted height: subtract header and footer heights approx (4rem + 4rem = 8rem) */}
                <ScrollArea className="h-[calc(100%-8rem)] p-4 space-y-4">
                  {messages.map(msg => (
                    <div
                      key={msg.id}
                      className={cn(
                        "flex w-max max-w-[75%] flex-col gap-1 rounded-lg px-3 py-2 text-sm shadow",
                    ...
                    >
                      <p>{msg.content}</p>
                      <span className={cn("text-xs self-end opacity-70", msg.senderId === CURRENT_USER_ID ? "text-primary-foreground/70" : "text-muted-foreground")}>
              ...
              <CardFooter className="border-t pt-4 p-4">
                <form onSubmit={handleSendMessage} className="flex w-full items-center space-x-2">
                  ...
                  />
                  <Button type="submit" size="icon" disabled={!newMessage.trim()} className="rounded-lg">
