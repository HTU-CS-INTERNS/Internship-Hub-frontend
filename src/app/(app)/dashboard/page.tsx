...
import { 
    LayoutDashboard, User, Briefcase, Building, CalendarCheck, FileTextIcon as FileTextLucide, MapPinIcon, StarIcon, ClockIcon, UploadCloudIcon, AlertOctagonIcon, UsersIcon, SchoolIcon, CheckCircle2, CircleDot, Circle, MessageSquare, PlusCircle
} from 'lucide-react';
...
const DashboardStatsCard: React.FC<{ title: string; value: string; icon: React.ElementType; progress?: number; progressLabel?: string; detail?: string; iconBgColor?: string; iconColor?: string }> = ({ title, value, icon: Icon, progress, progressLabel, detail, iconBgColor = "bg-primary-foreground", iconColor = "text-primary" }) => (
...
const TaskItem: React.FC<{ title: string; description: string; status: 'Completed' | 'Pending' | 'Overdue'; dueDate: string, statusIcon?: React.ElementType }> = ({ title, description, status, dueDate, statusIcon: StatusIcon = Circle }) => {
  const statusColors = {
    ...
    Pending: 'bg-yellow-100 text-yellow-700 border-yellow-500/30',
    Overdue: 'bg-red-100 text-red-700 border-red-500/30',
  };
  ...
  const Icon = status === 'Completed' ? CheckCircle2 : status === 'Pending' ? CircleDot : Circle;

  return (
    <div className="p-4 flex items-start">
        <div className="flex-shrink-0 mt-1">
            <Icon className={`h-5 w-5 ${status === 'Completed' ? 'text-green-500' : status === 'Pending' ? 'text-yellow-500' : 'text-red-500'}`} />
        </div>
...
const FeedbackItem: React.FC<{name: string; role: string; comment: string; time: string; avatarUrl?: string; avatarFallback: string; icon: React.ElementType; status?: 'Approved' | 'Pending'}> = ({name, role, comment, time, avatarUrl, avatarFallback, icon: Icon, status}) => {
    ...
    <div className="flex items-start">
        <Avatar className="h-10 w-10">
            ...
            <AvatarFallback className="bg-primary-foreground text-primary">{avatarFallback}</AvatarFallback>
        </Avatar>
        <div className="ml-3">
  ...
  return (
    <div className="space-y-6 p-4 md:p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <DashboardStatsCard title="Days Completed" value="14" icon={CalendarCheck} progress={15.5} progressLabel="Total Days: 90" iconBgColor="bg-blue-100 dark:bg-blue-900" iconColor="text-blue-600 dark:text-blue-300" />
        <DashboardStatsCard title="Reports Submitted" value="12" icon={FileTextLucide} detail="Approved: 10 (83.3%)" iconBgColor="bg-green-100 dark:bg-green-900" iconColor="text-green-600 dark:text-green-300" />
        <DashboardStatsCard title="Check-in Rate" value="92%" icon={MapPinIcon} detail="Last Check-in: Today (On Time)" iconBgColor="bg-orange-100 dark:bg-orange-900" iconColor="text-orange-600 dark:text-orange-300" />
        ...
        } iconBgColor="bg-yellow-100 dark:bg-yellow-900" iconColor="text-yellow-500 dark:text-yellow-300" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                    <div className="flex-1">
                     ...
                     <div className="flex-1">
                        ...
                        <Input id="attachment-file" type="file" className="text-xs"/>
                    </div>
                </div>
                <div>
                    <label htmlFor="work-summary" className="block text-sm font-medium text-foreground mb-1">Work Summary</label>
                    <Textarea id="work-summary" value={reportSummary} onChange={(e) => setReportSummary(e.target.value)} placeholder="Briefly describe what you accomplished..." rows={3} />
                </div>
        ...
        <div className="space-y-6">
                ...
                <ChartContainer config={checkInChartConfig} className="mx-auto aspect-square h-[120px]">
                  <PieChart accessibilityLayer>
                    ...
                    <Pie data={checkInChartData} dataKey="value" nameKey="name" innerRadius={35} outerRadius={50} strokeWidth={2}>
                     ...
                     <text x="50%" y="50%" textAnchor="middle" dominantBaseline="middle" className="fill-foreground text-lg font-bold progress-ring-text">
                        92%
                     </text>
                  </PieChart>
                </ChartContainer>
                <div className="text-center mt-2">
                  <h4 className="font-medium text-base">On-site Attendance</h4>
          ...
          <Card className="shadow-lg rounded-xl overflow-hidden">
            ...
            <CardContent className="p-0 divide-y">
                <DeadlineItem title="Final Project Submission" dueText="Due in 3 days" date="June 15, 2024" icon={AlertOctagonIcon} iconBg="bg-red-100 dark:bg-red-900" iconColor="text-red-600 dark:text-red-300" />
                <DeadlineItem title="Mid-term Evaluation" dueText="Due in 7 days" date="June 19, 2024" icon={FileTextLucide} iconBg="bg-yellow-100 dark:bg-yellow-900" iconColor="text-yellow-600 dark:text-yellow-300" />
                <DeadlineItem title="Team Presentation" dueText="Due in 10 days" date="June 22, 2024" icon={UsersIcon} iconBg="bg-blue-100 dark:bg-blue-900" iconColor="text-blue-600 dark:text-blue-300" />
            </CardContent>
          </Card>
          ...
          <Card className="shadow-lg rounded-xl overflow-hidden">
            ...
            <CardContent className="p-4 space-y-4">
                <FeedbackItem name="Mr. Smith" role="Company Supervisor" comment="Great progress on the documentation. Just a few minor corrections needed on section 3.2." time="2 hours ago" avatarFallback="MS" icon={Briefcase} status="Approved"/>
                <FeedbackItem name="Prof. Johnson" role="Faculty Lecturer" comment="Please provide more details about the team collaboration aspect in your next report." time="1 day ago" avatarFallback="PJ" icon={SchoolIcon} status="Pending"/>
            </CardContent>
          </Card>
        </div>
      </div>
